...
Usage:
    python merge_dataset_mac.py --start 2020-01-01 --end 2025-11-28 --dataset-a "data/ml/dataset_a_interim" --dataset-b "data/ml/dataset_b.parquet"
"""
...
class DatasetPreparer:
    ...
    def check_dataset_a_coverage(self, path: str) -> Dict:
            """
            Check Dataset A coverage (Folder/Partition Support).
            """
            path_obj = Path(path)
            ...
            # 1. Identify Files
            if path_obj.is_dir():
                files = sorted(list(path_obj.glob("*.parquet")))
                is_directory = True
                logger.info(f"Detected partitioned dataset: {len(files)} files")
            else:
                files = [path_obj]
                is_directory = False

            if not files:
                return {'exists': False, 'error': 'No parquet files found in directory'}

            # 2. Estimate Metadata (Fast Scan)
            total_rows = 0
            min_dates = []
            max_dates = []
            
            # We scan the first few files to get date ranges (assuming consistency)
            # and sum the row counts from metadata
            import pyarrow.parquet as pq
            
            for i, f in enumerate(files):
                try:
                    # Get row count from metadata (instant)
                    meta = pq.read_metadata(f)
                    total_rows += meta.num_rows
                    
                    # Check dates only on the first file to save time
                    # (Assuming all batches cover roughly the same period)
                    if i == 0:
                        df_dates = pd.read_parquet(f, columns=['date'])
                        min_dates.append(df_dates['date'].min())
                        max_dates.append(df_dates['date'].max())
                except Exception as e:
                    logger.warning(f"Could not read metadata for {f}: {e}")

            # Derive aggregations
            min_date = min(min_dates) if min_dates else pd.NaT
            max_date = max(max_dates) if max_dates else pd.NaT
            ...
            return {
                ...
                'range_start_covered': min_date <= self.start_date if pd.notnull(min_date) else False,
                'range_end_covered': max_date >= self.end_date if pd.notnull(max_date) else False,
                'missing_values_pct': 0, # Skipped for speed
    ...
    def merge_datasets(self, dataset_a_path: str, dataset_b_path: str, output_path: str) -> pd.DataFrame:
            """
            Merge Dataset B with a Folder of Dataset A partitions.
            """
            logger.info("Merging datasets (Folder Iteration Strategy)...")
            
            # 1. Load Dataset B (Targets)
            # We need this in memory to match against
            df_b = pd.read_parquet(dataset_b_path)
            
            # Ensure join keys are consistent types
            df_b['ticker'] = df_b['ticker'].astype(str)
            df_b['entry_date'] = pd.to_datetime(df_b['entry_date'])
            
            logger.info(f"Loaded {len(df_b)} trade targets from Dataset B")
            
            # 2. Identify Dataset A Parts
            path_obj = Path(dataset_a_path)
            if path_obj.is_dir():
                files = sorted(list(path_obj.glob("*.parquet")))
            else:
                files = [path_obj]
                
            logger.info(f"Found {len(files)} partitions in Dataset A to process")
            ...
            for i, file_path in enumerate(files):
                try:
                    # Load one partition (e.g., 100 tickers)
                    # This is memory safe because we limited batch size in build_dataset_a
                    ...
                    rows_processed += len(df_part)
                    
                    # INNER JOIN
                    # Find rows in this partition that match our trade list
                    match = pd.merge(
                        df_b,
                        df_part,
                    ...
                    if not match.empty:
                        merged_chunks.append(match)
                        # Optional: Print if we found something (satisfying to watch)
                        # logger.info(f"  Found {len(match)} matches in {file_path.name}")
            ...
            if not merged_chunks:
                logger.error("No matches found! Check your date ranges and ticker symbols.")
                return pd.DataFrame()

            logger.info("Concatenating all matches...")
            final_df = pd.concat(merged_chunks, ignore_index=True)
            
            # Clean up column duplication from merge (if any)
            # e.g., if date and entry_date are both kept
            if 'date' in final_df.columns and 'entry_date' in final_df.columns:
                # Usually we keep entry_date as the source of truth for the trade
                pass 
            ...
            self.merged_dataset = final_df
            self.validation_report['merge_stats'] = {
                'merged_rows': len(final_df),
                'total_columns': len(final_df.columns),
                'match_rate_pct': (len(final_df) / len(df_b)) * 100,
                'missing_snapshots': len(df_b) - len(final_df)
            }
    ...
    def generate_report(self) -> str:
            ...
            if 'dataset_a' in self.validation_report:
                ...
                if da.get('exists'):
                    report_lines.append(f"✅ Path found: {da['path']}")
                    if da.get('is_directory'):
                        report_lines.append(f"   Type: Partitioned Dataset (Folder)")
                        report_lines.append(f"   Partitions: {da.get('file_count', 'N/A')}")
                    else:
                        report_lines.append(f"   Type: Single File")

                    # Handle potentially missing dates (if folder scan failed)
                    min_d = da.get('min_date')
                    max_d = da.get('max_date')
                    min_str = min_d.date() if isinstance(min_d, (pd.Timestamp, datetime)) else "Unknown"
                    max_str = max_d.date() if isinstance(max_d, (pd.Timestamp, datetime)) else "Unknown"

                    report_lines.append(f"\nDate Coverage:")
                    report_lines.append(f"  Available: {min_str} to {max_str}")
                    report_lines.append(f"  Start Date: {'✅ COVERED' if da.get('range_start_covered') else '❌ NOT COVERED'}")
                    report_lines.append(f"  End Date: {'✅ COVERED' if da.get('range_end_covered') else '❌ NOT COVERED'}")
                    
                    # FIX: Handle missing keys gracefully
                    cov_pct = da.get('coverage_pct', 'N/A')
                    if isinstance(cov_pct, (int, float)):
                        cov_str = f"{cov_pct:.1f}%"
                    else:
                        cov_str = str(cov_pct)
                        
                    missing_dates = da.get('missing_dates_count', 'N/A')
                    report_lines.append(f"  Coverage: {cov_str} ({missing_dates} missing dates)")
                    
                    report_lines.append(f"\nData Statistics:")
                    report_lines.append(f"  Total Rows: {da.get('total_rows', 0):,}")
                    report_lines.append(f"  Features: {da.get('total_features', 0)}")
                else:
            ...
            if 'dataset_b' in self.validation_report:
                ...
                if db.get('exists'):
                    ...
                    report_lines.append(f"  Win Rate: {db.get('win_rate', 0)*100:.1f}%")
                    
                    if db.get('label_distribution'):
                        report_lines.append(f"\nLabel Distribution:")
            ...
            if 'sanity_checks' in self.validation_report:
                ...
                # Individual checks
                report_lines.append(f"\nDetailed Checks:")
                if 'duplicate_check' in sc:
                    report_lines.append(f"  Duplicate Rows: {status_icon[sc['duplicate_check']]} {sc['duplicate_check']} ({sc.get('duplicate_rows', 0)} duplicates)")
                if 'critical_columns_check' in sc:
                    report_lines.append(f"  Critical Columns: {status_icon[sc['critical_columns_check']]} {sc['critical_columns_check']}")
                if 'missing_values_check' in sc:
                    report_lines.append(f"  Missing Values: {status_icon[sc['missing_values_check']]} {sc['missing_values_check']} ({sc.get('missing_values_pct', 0):.2f}%)")
