...
def main():
    ...
    # Load benchmark data with HISTORICAL mode (validates date range, NOT latest trading day)
    benchmark_data = data_repo.get_benchmark_data(
        mode=CacheMode.HISTORICAL,
        date_range=(args.start, outcome_end)
    )

    if benchmark_data is None:
        logger.error("Failed to load benchmark data!")
        logger.error(f"Make sure SPY data is cached and covers {args.start} to {outcome_end}")
